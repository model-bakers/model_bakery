...
class TestFullClean:
    ...
    @pytest.mark.django_db
    def test_make_with_full_clean_validates_generated_related_objects(self):
        """_full_clean=True validates autogenerated related FK objects."""
        with pytest.raises(ValidationError):
            baker.make(
                models.PaymentBill,
        ...
        assert models.PaymentBill.objects.count() == 0
    ...
    @pytest.mark.django_db
    def test_bulk_create_with_full_clean_validates_generated_related_objects(self):
        """_full_clean=True with _bulk_create=True validates generated FKs."""
        with pytest.raises(ValidationError):
            baker.make(
                models.PaymentBill,
                user__profile__email="not-an-email",
    ...
    @pytest.mark.django_db
    def test_bulk_create_with_full_clean_does_not_validate_existing_related_objects(
        self,
    ):
        profile = baker.make(models.Profile, email="not-an-email")
        users = baker.make(
            models.User,
            profile=profile,
            _quantity=2,
